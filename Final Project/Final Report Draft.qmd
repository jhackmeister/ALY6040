...
From this, we can see a very strong correlation between AC count and PCOS, menstrual irregularities and testosterone levels are also strong correlations, while age and BMI so far weaker relationships.

To make analysis easier we'll convert PCOS diagnosis and menstrual irregularities to factor variables with yes/no labels.
...
Next, we'll take a look at the distribution of all the variables in the dataset.
...
To better understand the relationship between the variables and a PCOS diagnosis, we'll split the dataset into yes and no groups.
...
Then will construct a series of plots to see how they interact with the diagnosis.
...
First, we will create the model using the e1701 library and review our results.
...
Now we'll use the model to make predictions on our dataset.
...
This shows very strong predictive strength for the model with only 1 false positive and 2 false negatives. To further examine the strength of the model, we'll partition the dataset into training and test sets.
...
With these new datasets, we'll refit the model and test our results.
